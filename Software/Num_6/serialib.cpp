...
char serialib::openDevice(const char *Device, const unsigned int Bauds,
    ...
    // test if the device is not locked
    char s[100]; sprintf(s,"%s", Device);
    for (int i=0; i<strlen(s); i++) if (s[i]=='/') s[i]='_';
    sprintf(lockfile,"/tmp/%s", s);  //printf("lockfile=%s ", lockfile);
    FILE *lf;
    lf=fopen(lockfile, "r");
    if ( lf!= NULL) {fclose(lf); /*printf("%s locked\n", lockfile);*/ return -10; }
    ...
    // lock it
    lf=fopen(lockfile, "w");
    if (lf==NULL) {/*printf("cannot lock file\n");*/return -11; }
    fprintf(lf,"locked");
    fclose(lf);
    system("sync");
...
char serialib::readChar(char *pByte,unsigned int timeOut_ms)
{
    ...
    // Set the TimeOut
    timeouts.ReadTotalTimeoutConstant=timeOut_ms; // MAXDWORD
